...
We introduce NVV-SuperBench, a standardized evaluation suite for assessing a speech generation system's ability to synthesize _nonverbal vocalizations (NVVs)_ beyond lexical content. The overview of NVV-SuperBench is presented in Figure 1. Given an input utterance, NVV-SuperBench supports two commonly used NVV-control interfaces: (i) prompt-based control, where NVV cues are specified in a natural-language caption; and (ii) tag-based control, where NVV tags (e.g., \[laugh\], \[sigh\]) are inserted into text. A candidate speech generation system generates speech conditioned on the input, and NVV-SuperBench evaluates the speech output from three complementary perspectives: automatic objective metrics, human subjective listening tests, and LLM-based multi-rater evaluation.
...
The objective results for both prompt-based and tag-based systems are summarized in Table 2.4.2. Across three independent synthesis runs, most measures exhibit small run-to-run variance, indicating that the systems are _stable_.
