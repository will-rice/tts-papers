...
Denoising. The triplet $`\langle text,audio\textsubscript{augment},audio\textsubscript{source}\rangle`$ is constructed for denoising, with audio*(source) serving as the ground-truth reference and audio*(augment) being generated through additive noise and reverberation simulation.

Silence Trimming. The triplet is defined as $`\langle text,audio\textsubscript{source},audio\textsubscript{trimming}\rangle`$, where audio*(source) corresponds to the source audio containing silent segments, and audio*(trimming) refers to the processed version generated by extracting and concatenating speech segments according to the timestamps produced by Silero-VAD\[SileroVAD\].
