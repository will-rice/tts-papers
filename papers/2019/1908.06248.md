...
- •
  $`F_{0}`$ range (gender_f0range.txt): Typical pitch extractors, e.g., \[19, 20, 21\], have a range for $`F_{0}`$ search, and the setting is critical for the results ultimately obtained for the voices. This corpus contains manually annotated $`F_{0}`$ ranges per speaker for his/her normal voices.
- •
