...
Descript-audio-codec (DAC) \[[13](https://arxiv.org/html/2409.04016v1#bib.bib13)\] is a recent codec system that features several VQ improvements over Encodec. DAC mitigates codebook collapse by quantizing in a very low-dimensional latent space. Its updated RVQ is: $`Q_{n} = {\text{Proj\_Out}{({\text{VQ}{({{\text{Proj\_In}{(Q_{0})}} - {\sum_{i = 1}^{n - 1}{\text{Proj\_In}{(Q_{i})}}}})}})}}`$ where Proj_In is a linear projection from the original latent space (1024 dimensional) to the low-dimensional quantization latent space (8 or 32 dimensional). It also changes the VQ lookup distance from Euclidean distance to cosine similarity for stability. DAC uses an explicit MSE codebook loss function instead of the EMA update scheme to learn the projection functions. This loss can be expressed as $`L_{codebook} = {\|{q - z}\|}_{2}^{2}`$, where $`z`$ and $`q`$ denote the looked-up vectors and the quantized vectors, respectively.
