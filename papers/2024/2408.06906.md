...
where$`|| \cdot ||_{F}`$denote the Frobenius norms and S denotes the number of elements in the spectrogram. Each m-th L_(FM) reuse $`S_{m}`$ and $`\hat{S_{m}}`$ used in the m-th MTD sub-discriminator. The number of each loss is M, which is the same as the number of MTD sub-discriminators.
...
where$`|| \cdot ||_{1}`$ denotes the L1 norms, and S denotes the number of elements in the spectrogram. Each m-th L_(FM) reuse $`S_{m}`$ and $`\hat{S_{m}}`$ used in the m-th MTD sub-discriminator. The number of each loss is M, which is the same as the number of MTD sub-discriminators.
