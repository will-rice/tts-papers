...
The key difference lie in Figure [5](https://arxiv.org/html/2407.14212v1#S3.F5 "Figure 5 ‣ 3.4 Parameter Settings and Pre-training Process ‣ 3 Method ‣ Braille-to-Speech Generator: Audio Generation Based on Joint Fine-Tuning of CLIP and Fastspeech2") is that Figure [9](https://arxiv.org/html/2407.14212v1#S4.F9 "Figure 9 ‣ 4.3.1 Convergence Analysis ‣ 4.3 Ablation Experiments ‣ 4 Experiments ‣ Braille-to-Speech Generator: Audio Generation Based on Joint Fine-Tuning of CLIP and Fastspeech2") depicts more iteration rounds are required for individual vocoder pre-training. After fine-tuning, the loss trends of the four models stabilize and converge after 50 epochs, indicating that the efficiency of step-wise pre-training followed by joint fine-tuning is higher than directly joint training. From the overall loss trend graph, it can be observed that the model CLIP_(ViT-H/14)-KNN-Fastspeech2 has the fastest convergence speed and the smallest convergence loss, while the model CLIP_(ViT-H/14)-KNN-Tacotron2 has the slowest convergence speed and the largest convergence loss. Zooming in on the details from epoch 20 to 50:

(1) The model CLIP$`{ViT} - {H/14}`$-KNN-Fastspeech2 starts to converge after 25 iterations,which is 10 epochs earlier than the model CLIP_(ViT-H/14)-KNN-Tacotron2. (2) The convergence time points and convergence loss values of the models CLIP_(ViT-L/14)-KNN-Fastspeech2 and CLIP_(ViT-H/14)-KNN-Fastspeech2 are consistent, while they differ from the convergence loss of the model CLIP_(ViT-B/16)-KNN-Fastspeech2.
...
Figure 10: Count of Ratings (The numbers in the figure represent models 1: CLIP_(ViT-H/14)-KNN-Tacotron2, 2: CLIP_(ViT-B/16)-KNN-Fastspeech2, 3: CLIP_(ViT-L/14)-KNN-Fastspeech2, 4: CLIP_(ViT-H/14)-KNN-Fastspeech2, and the numbers in the figure represent the number of ratings for each level.)
...
According to the objective loss function $`Loss_{total}`$, the numerical magnitude of $`\lambda_{1}`$ and $`\lambda_{2}`$ indirectly reflects the contribution of I2T and T2A to the final speech synthesis capability in the CLIP-KNN-Fastspeech2 model. We will select the optimal model CLIP_(ViT-H/14)-KNN-Fastspeech2 in Table 4 for comparative analysis of WER indicators on the test set of BIT-DP,Wukong.At the same time, we also analyzed the changes in FAD values under different parameter combinations on the datasets VGGSound and Flickr8k. As shown in the Figure [12](https://arxiv.org/html/2407.14212v1#S4.F12 "Figure 12 ‣ 4.4.3 Weight Parameters Analysis ‣ 4.4 Futher Analysis ‣ 4 Experiments ‣ Braille-to-Speech Generator: Audio Generation Based on Joint Fine-Tuning of CLIP and Fastspeech2").
