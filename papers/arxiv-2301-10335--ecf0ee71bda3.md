...
Next, we attempt to decorrelate accent and speaker variables from _each other_ by minimizing the cross-correlation matrix from batch statistics. Let $`\overset{\sim}{E^{A}}`$ and $`{\overset{\sim}{E}}^{S}`$ be the sampled column matrices of accent and speaker embedding vectors sampled within a batch of size $`B`$. We compute the batch cross -correlation matrix $`R^{A\hspace{0pt}S}`$ as follows ($`\mu_{E^{A}}`$ and $`\mu_{E^{S}}`$ computed from embedding table):
