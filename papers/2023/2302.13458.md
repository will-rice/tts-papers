---
arxiv_id: '2302.13458'
title: 'Varianceflow: High-Quality and Controllable Text-to-Speech using Variance
...
The invertibility of NF makes it possible to obtain the raw variance value $`\mathbf{x}`$ from the latent representation $`\mathbf{z}`$. Therefore, it is also possible for VarianceFlow to control the variance factors as other Type-II TTS models (Figure 2). To control the variance information, the sampled latent representations are first brought to the raw variance space using the inverse transforms of NF. Then, the variance factor is manipulated in the space (*e.g.*, multiplying a constant to the raw variance value $`\mathbf{x}`$). Lastly, the manipulated value is provided to the FFT decoder via NF again.
