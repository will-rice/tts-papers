...
1.  1.
    The primary contribution is the novel CSS framework CONCSS for enhancing prosody, driven by contrastive learning to address the context understanding issue from a self-supervised perspective. To the best of our knowledge, we are the first to adopt contrastive learning for the CSS task.
2.  2.
    To address the issue of context representation in CSS, we design an innovative pretext task specific to CSS tasks, along with a sampling strategy. This approach compels the context encoder to generate distinct representations for diverse scenarios, thereby promoting both context sensitivity and distinctiveness in prosody.
3.  3.
    We comprehensively evaluate models on their ability to produce context-sensitive vectors and dialogue-appropriate prosody. Specifically, we compare prosodic performance between different CSS frameworks when they are exposed to diverse contexts.
