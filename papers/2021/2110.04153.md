...
Fig. 2: The SV embeddings of different models, each point corresponds to one SV embedding. ’$`▶`$’ and ’$`+`$’ denote *target speaker’s* and *source speaker’s* SV embedding points respectively, ’$`\star`$’ denotes SV embeddings of synthesized speech and colors represent different emotions.
