...
|  |  |  |  |  |  |  |  |  |
|----|----|----|----|----|----|----|----|----|
|   Model | MOS $`( \uparrow )`$ | MCD₁₃ $`( \downarrow )`$ | RMSE_(f0) $`( \downarrow )`$ | FDSD $`( \downarrow )`$ | Speed on CPU $`( \uparrow )`$ |  | Speed on GPU $`( \uparrow )`$ |  |
...
We assessed Fre-GAN on various metrics. As a subjective test, we performed 5-scale MOS tests via Amazon MTurk where at least 20 raters were asked to rate the naturalness of audio. As an objective quality evaluation, we used MCD₁₃\[34\], RMSE_(f0)\[35\], and FDSD\[36\]. 50 and 200 synthesized utterances were used for subjective and objective evaluation, respectively. We also measured the synthesis speed on Intel Xeon Gold 6148 2.40 GHz CPU and a single NVIDIA Titan Xp GPU.
...
- \[9\] K. Kumar, R. Kumar, T. de Boissiere, L. Gestin, W. Z. Teoh, J. Sotelo, A. de Brébisson, Y. Bengio, and A. Courville, “MelGAN: Generative Adversarial Networks for Conditional Waveform Synthesis,” in *Advances in Neural Information Processing Systems (NeurIPS)*, 2019.
