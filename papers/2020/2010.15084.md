...
Fig. 1: Comparison of two existing models that use factors of variation such as rhythm to affect the audio generation. Red blocks are learned models, yellow blocks are fixed operations. (a) Mellotron \[2\] does not infer the factors of variation, it always requires them as input. (b) FastSpeech \[3\] infers phoneme durations (i.e. rhythm) and offers (but _does not require_) their control. Offering control without requiring it makes a model applicable in a wider variety of settings.
