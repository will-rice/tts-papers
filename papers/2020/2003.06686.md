---
arxiv_id: '2003.06686'
title: Perception of prosodic variation for speech synthesis using an unsupervised
...
In simple terms, the VAMP prior is a Gaussian mixture model (GMM), whose parameters are learned jointly with the rest of the model. However, we do not learn the GMM parameters directly, instead we learn $`K`$ ‘*pseudo-inputs*’ $`\left\{ \mathbf{u}_{k} \right\}_{k = 1}^{K}`$, where $`K`$ is a hyperparameter. These *pseudo-inputs* are not real inputs, they are parameters learned through backpropagation. Each GMM component is given by a *pseudo-input*’s approximate posterior $`{p\hspace{0pt}\left( {\mathbf{z}_{k} \mid \mathbf{u}_{k}} \right)} = {\mathcal{N}\hspace{0pt}\left( \mathbf{z}_{k};{\mathbf{μ}}_{\mathbf{u}_{k}},{\mathbf{σ}}_{\mathbf{u}_{k}} \right)}`$. Here, we define our *intonation codes* using GMM component centres $`{\mathbf{μ}}_{\mathbf{u}_{k}}`$.
...
- \[4\] J. Kleinhans, M. Farrús, A. Gravano, J. M. Pérez, C. Lai, and L. Wanner, “Using prosody to classify discourse relations,” in *Proc. Interspeech*, 2017, pp. 3201–3205.
...
- \[9\] S. Betz, S. Zarrieß, E. Székely, and P. Wagner, “The Greennn Tree — Lengthening Position Influences Uncertainty Perception,” in *Proc. Interspeech*, 2019, pp. 3990–3994.
...
- \[21\] S. Tyagi, M. Nicolis, J. Rohnke, T. Drugman, and J. Lorenzo-Trueba, “Dynamic prosody generation for speech synthesis using linguistics-driven acoustic embedding selection,” *arXiv preprint arXiv:1912.00955*, 2019.
