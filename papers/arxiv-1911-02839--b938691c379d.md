...
The teacher decoder, Decoder*T outputs a hidden state $`s*{t}`$ at each step $`t`$:
...
Similarly, the student decoder Decoder*S processes the same input sequence and generates the hidden state $`{\hat{s}}*{t}`$ at each step $`t`$ at the same time:
