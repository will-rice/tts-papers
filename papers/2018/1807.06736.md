...
Some techniques have been proposed to improve the performance of original attention mechanism. One is *adding convolutional features* \[15\] for stabilizing the attention alignment. In detail, $`k`$ filters with kernel size $`l`$ are employed to convolve the alignment of previous decoder timestep. Let $`{\mathbf{F}} \in {\mathbb{R}}^{k \times l}`$ represent the convolution matrix. Then it is used as an extra term for calculating the attention probabilities and we have
