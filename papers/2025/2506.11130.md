...
For ASR model $`\mathcal{F}_{\theta}`$ that accepts audio inputs up to $`L_{\text{max}}`$ seconds per inference window (e.g., 30 seconds for Whisper), clips exceeding this duration are classified as *long-form*. Long-form audio modeling requires segmenting the input into shorter chunks to satisfy the model’s input constraints. Training the model solely on short utterances (typically 5 to 25 seconds) creates a distributional mismatch between the training data and the $`L_{\text{max}}`$-second segments encountered during long-form transcription and degrades performance. To address this challenge, we augment $`L_{max}`$-second audios from existing samples to simulate chunked long-form audio.
