...
Implicit Alignment Mechanism. We propose an *Implicit Alignment Mechanism* to avoid costly phoneme-level duration labels. Each phoneme inherits its character’s temporal span, with known start time $`t_{\text{start}}`$ and duration $`d_{\text{char}}`$, extended backward by a tunable offset $`\delta`$:
