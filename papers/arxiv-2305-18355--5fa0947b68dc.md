...
arxiv_id: "2305.18355"
title:
  An Efficient Membership Inference Attack for the Diffusion Model by Proximal
  Initialization
authors:
  - Fei Kong
  - Jinhao Duan
...
Generative Diffusion Models   Generative diffusion models have recently achieved significant success in both image Ramesh2022Hierarchical ; stable*diffusion and audio generation tasks Huang2022FastDiff ; Chen2021WaveGrad ; Popov2021Grad-TTS . Unlike GANs gans ; yuan2020attribute ; yuan2023dde , which consist of a generator and a discriminator, diffusion models generate samples by fitting the inverse process of a diffusion process from Gaussian noise. Compared to GANs, diffusion models typically produce higher quality samples and avoid issues such as checkerboard artifacts salimans2016improved ; Donahue2017Adversarial ; Dumoulin2017Adversarially . A diffusion process is defined as $`{{\mathbf{x}}*{t} = {{\sqrt{\alpha_{t}}\hspace{0pt}{\mathbf{x}}_{t - 1}} + {\sqrt{\beta_{t}}\hspace{0pt}\mathbf{\epsilon}_{t}}}},{\mathbf{\epsilon}_{t} \sim {\mathcal{N}\hspace{0pt}{(\mathbf{0},\mathbf{I})}}}`$, where $`{\alpha*{t} + \beta*{t}} = 1`$ and $`\beta*{t}`$ increases gradually as $`t`$ increases, so that eventually, $`{\mathbf{x}}*{t}`$ approximates a random Gaussian noise. In the reverse diffusion process, $`{\mathbf{x}}_{t}'`$ still follows a Gaussian distribution, assuming the variance remains the same as in the forward diffusion process, and the mean is defined as $`{\overset{\sim}{\mathbf{μ}}}_{t} = {\frac{1}{\sqrt{a*{t}}}\hspace{0pt}\left( {{\mathbf{x}}*{t} - {\frac{\beta*{t}}{\sqrt{1 - {\overline{a}}*{t}}}\hspace{0pt}{\overline{\mathbf{\epsilon}}}_{\theta}\hspace{0pt}{({\mathbf{x}}_{t},t)}}} \right)}`$, where $`{\overline{\alpha}}_{t} = {\prod_{k = 0}^{t}\alpha*{k}}`$ and $`{{\overline{\alpha}}*{t} + {\overline{\beta}}_{t}} = 1`$. The reverse diffusion process becomes $`{{\mathbf{x}}_{t - 1} = {{\overset{\sim}{\mathbf{μ}}}_{t} + {\sqrt{\beta_{t}}\hspace{0pt}\mathbf{\epsilon}}}},{\mathbf{\epsilon} \sim {\mathcal{N}\hspace{0pt}{(\mathbf{0},\mathbf{I})}}}`$. One can obtain a loss function Eq. 1 by minimizing the distance between the predicted and groundtruth distributions. Score_Based transforms the discrete-time diffusion process into a continuous-time process and uses SDE ( Stochastic Differential Equation) to express the diffusion process. To accelerate the generation process, several methods have been proposed, such as Salimans2022Progressive ; Dockhorn2022Score-Based ; Xiao2022Tackling . DDIM DDIM is another popular method that proposes a forward process different from diffusion process with the same loss function as DDPM, allowing it to reuse the model trained by DDPM while achieving higher generation speed.
...
Recently, some diffusion models are trained with continuous time. As demonstrated in Score*Based , the diffusion process with continuous time can be defined by a stochastic differential equation (SDE) as $`{d\hspace{0pt}{\mathbf{x}}*{t}} = {{{\mathbf{f}}_{t}\hspace{0pt}{({\mathbf{x}}_{t})}\hspace{0pt}d\hspace{0pt}t} + {g*{t}\hspace{0pt}d\hspace{0pt}{\mathbf{w}}*{t}}}`$, where $`{\mathbf{w}}_{t}`$ is a Brownian process. One of the reverse process is $`{d\hspace{0pt}{\mathbf{x}}_{t}} = {{\left( {{{\mathbf{f}}_{t}\hspace{0pt}{({\mathbf{x}}_{t})}} - {\frac{1}{2}\hspace{0pt}\left( {g*{t}^{2} + \sigma*{t}^{2}} \right)\hspace{0pt}{{\nabla_{{\mathbf{x}}_{t}}\log}p_{t}}\hspace{0pt}{({\mathbf{x}}_{t})}}} \right)\hspace{0pt}d\hspace{0pt}t} + {\sigma_{t}\hspace{0pt}d\hspace{0pt}{\mathbf{w}}}}`$. When $`\sigma*{t} = 0`$, this formula becomes an ordinary differential equation (ODE): $`{d\hspace{0pt}{\mathbf{x}}*{t}} = {\left( {{{\mathbf{f}}_{t}\hspace{0pt}{({\mathbf{x}}_{t})}} - {\frac{1}{2}\hspace{0pt}g*{t}^{2}\hspace{0pt}{{\nabla*{{\mathbf{x}}_{t}}\log}p_{t}}\hspace{0pt}{({\mathbf{x}}_{t})}}} \right)\hspace{0pt}d\hspace{0pt}t}`$. Continuous-time diffusion model train an $`{\mathbf{s}}_{\mathbf{θ}}`$ to approximate $`{{\nabla_{{\mathbf{x}}_{t}}\log}p_{t}}\hspace{0pt}{({\mathbf{x}}\_{t})}`$, so the loss function will be:
