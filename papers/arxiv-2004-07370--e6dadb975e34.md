...
where $`\lambda`$ is a tunable hyperparameter. As shown in \[17\], if $`{\mathbf{c}}\hspace{0pt}\left\lbrack n' \right\rbrack`$ has a proper dimension and is properly downsampled, and given some other assumptions, AutoVC can achieve “perfect conversion”, in the sense that the conversion output would match the true distribution of the target speaker uttering the source content. This is because the narrow bottleneck can squeeze out the source speaker information and keep the content information only, forcing _disentanglement_ between speaker and content information.
