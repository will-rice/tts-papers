...
1.  1.  Reference extraction: We isolate a 20-second speech segment from the reference audio using energy-based Voice Activity Detection (VAD).
2.  2.  Text chunking: Long inputs are split into segments of up to 200 characters at sentence boundaries.
3.  3.  Synthesis: Each chunk is processed with a temperature of 0.8 and top-p of 0.9, and the resulting audio segments are concatenated.
