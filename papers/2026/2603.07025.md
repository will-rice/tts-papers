...
Building on prior Speech LLM models, we train a lightweight adapter using paired speech and transcript data. Given speech $`x`$ and transcript $`y`$, the model learns a cross-modal projector that converts speech embeddings into *text-like* representations that act as a *soft speech prefix* for the frozen LLM to generate text.
