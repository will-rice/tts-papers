...
The 292-dimensional latent is split into a 256-dimensional *semantic* component and a 36-dimensional *acoustic* component, which are quantized independently:
...
Here, $`D_{n}^{m}`$ denotes the $`m`$-th layer of the $`n`$-th discriminator, where each of the $`N`$ discriminators has $`M`$ layers. Following Défossez et al. \[2024\], Parker et al. \[2024\], we use this feature-matching loss *in place of* the standard GAN generator loss, as the evolving discriminator features provide an increasingly discriminative reconstruction signal throughout training.
...
1.  1.
    Word Error Rate (WER): Measured by Voxtral Mini Transcribe v2 to capture the intelligibility of speech.
2.  2.
