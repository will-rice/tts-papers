...
Two-stage methods decompose enhancement into semantic and acoustic stages \[65, 69, 29\]. These approaches rely on semantic representations¹¹1Also referred to as *phonetic* representations, as they exhibit properties closer to phonetic-like units. \[4, 41\] and acoustic representations, with the former typically derived from deep-layer SSL representations, while the latter can take various forms (e.g., codec tokens \[65\] and Mel spectrograms \[29\]) as long as they preserve sufficient fine-grained acoustic information for waveform reconstruction. The semantic stage generates conditions to guide the subsequent acoustic enhancement, often yielding superior performance over single-stage methods.
...
TABLE III: Detailed training configurations of each module in UniPASE. *DWO* denotes *DeWavLM-Omni*.
...
TABLE X: Pairwise CCR results on the URGENT 2025 *non-blind* test set. Results are reported as mean $`\pm`$ 95% confidence intervals (CI).
