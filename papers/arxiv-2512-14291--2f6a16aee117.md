...
1.  1.  The system first processes the entire input sentence through a Grapheme-to-Phoneme (G2P) module to generate a complete phoneme sequence (phoneme_list).
2.  2.  It iterates through the original text list; if a polyphone or rare character is encountered, the text is replaced with its corresponding phoneme from the generated list.
3.  3.  The final input to the model takes a “hybrid phoneme+text” format. This approach ensures precise pronunciation control for ambiguous characters while preserving the natural prosody associated with the standard text.
