...
arxiv_id: "2209.01250"
title:
  Improving Contextual Recognition of Rare Words with an Alternate Spelling Prediction
  Model
authors:
...
categories:
  ...
  - cs.SD
  - eess.AS
arxiv_url: https://arxiv.org/abs/2209.01250
source: arxiv-html
converter: pandoc
llm_remediated: false
citations_resolved: 0/0
citations_resolved_at: "2026-07-07T16:31:16+00:00"
references_parsed: 0
arxiv_version: ""
---
...
The maximum number of pages is 5. The 5^(th) page is reserved for references, exclusively. However, the references may begin on an earlier page immediately after the Acknowledgements section, and continue onto the 5^(th) page. If no space is available on an earlier page, then the references may begin on the 5^(th) page.
...
This template can be found on the conference website. Templates are provided for Microsoft Word®, and LaTEX. However, we highly recommend using LaTEXwhen preparing your submission. Information for full paper submission is available on the conference website.

## 2 Page layout and style

Authors should observe the following rules for page layout. A highly recommended way to meet these requirements is to use a given template (Microsoft Word® or LaTEX) and check details against the corresponding example PDF file. Given templates, Microsoft Word® or LaTEX, can be adapted/imported easily in other software such as LibreOffice, Apple Pages, LuaLaTEX, and XeLaTEX, but please be careful to match the layout of the provided PDF example.

### 2.1 Basic layout features

- •
  Proceedings will be printed in DIN A4 format. Authors must submit their papers in DIN A4 format.
- •
...
- •
  Check indentations and spacings by comparing to this example file (in PDF).
...
1.  1.  For Windows users, the bullzip printer can convert any PDF to have embedded and subsetted fonts.
2.  2.  For Linux/Mac users, you may use \
        pdftops file.pdf\
        pstopdf -dPDFSETTINGS=/prepress file.pdf

LaTEXusers: users should use Adobe Type 1 fonts such as Times or Times Roman. These are used automatically by the INTERSPEECH2021.sty style file. Authors must not use Type 3 (bitmap) fonts.

### 2.3 Figures

All figures must be centered on the column (or page, if the figure spans both columns). Figure captions should follow each figure and have the format given in Figure 1.
...
An example of a table is shown in Table 1. The caption text must be above the table.

| Ratio    |                        | Decibels      |
| -------- | ---------------------- | ------------- |
| $`1`$    | $`\left. /10 \right.`$ | $`- 20`$      |
| $`1`$    | $`\left. /1 \right.`$  | $`0`$         |
| $`2`$    | $`\left. /1 \right.`$  | $`\approx 6`$ |
| $`3.16`$ | $`\left. /1 \right.`$  | $`10`$        |
| $`10`$   | $`\left. /1 \right.`$  | $`20`$        |
| $`100`$  | $`\left. /1 \right.`$  | $`40`$        |
| $`1000`$ | $`\left. /1 \right.`$  | $`60`$        |
...
```math
{x\hspace{0pt}(t)} = {s\hspace{0pt}\left( {f_{\omega}\hspace{0pt}(t)} \right)}
```

where $`f_{\omega}\hspace{0pt}(t)`$ is a special warping function

```math
{f_{\omega}\hspace{0pt}(t)} = {\frac{1}{2\hspace{0pt}\pi\hspace{0pt}j}\hspace{0pt}\left. \oint{}_{C}\frac{\nu^{- {1\hspace{0pt}k}}\hspace{0pt}d\hspace{0pt}\nu}{\left( {1 - {\beta\hspace{0pt}\nu^{- 1}}} \right)\hspace{0pt}\left( {\nu^{- 1} - \beta} \right)} \right.}
```

A residue theorem states that

```math
\left. \oint{}_{C}{F\hspace{0pt}(z)\hspace{0pt}{dz}} \right. = {2\hspace{0pt}\pi\hspace{0pt}j\hspace{0pt}{\sum\limits_{k}{{Res}\hspace{0pt}\left\lbrack {F\hspace{0pt}(z)},p_{k} \right\rbrack}}}
```
...
![Refer to caption](https://ar5iv.labs.arxiv.org/html/2209.01250/assets/figure.pdf)

Figure 1: Schematic diagram of speech production.

### 2.6 Information for Word users only
...
For technical reasons, the proceedings editor will strip all active links from the papers during processing. Hyperlinks can be included in your paper, if written in full, e. g. “http://www.foo.com/index.html”. The link text must be all black. Please make sure that they present no problems in printing to paper.

### 2.8 Multimedia files
...
Final page numbers will be added later to the document electronically. _Do not make any footers or headers!_

### 2.10 References

The reference format is the standard IEEE one. References should be numbered in order of appearance, for example \[Davis80-COP\], \[Rabiner89-ATO\], \[Hastie09-TEO, pp. 417–422\], and \[YourName21-XXX\].
