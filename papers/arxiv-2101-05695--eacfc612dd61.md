...
1.  1.  It uses 64-dim emotion embeddings instead of 128-dim speaker embeddings (see Section 3.1).
2.  2.  It uses a gradient inverter block to remove emotion leakage from the bottleneck embeddings (see Section 3.2).
3.  3.  It operates on multi-lingual data (see Section 4.1).
4.  4.  It does not pass the phoneme embeddings to the VAE reference encoder.
...
1.  1.  Imagine there is no leakage in the input. As the classifier cannot rely on any information in the input, its prediction is random and the cross-entropy loss on its predictions is high. Thus the back-propagated gradients are large as well. Even though there is no leakage the preceding network receives a large reversed gradient.
