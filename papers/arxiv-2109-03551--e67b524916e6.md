...
1.  1.  The DTW algorithm minimizes a distance metric between the aligned source and target feature vectors. For mcep inputs, the mel-cepstrum distortion (MCD) is often used, whose definition is as follows:
...
2.  2.  Construct the aligned joint feature vectors with the estimated time-warping function.
3.  3.  Train a GMM with the joint feature vectors.
4.  4.  Convert the source mceps with the trained GMM.
5.  5.  Go to step 1 and replace the source mceps with the converted mceps.
