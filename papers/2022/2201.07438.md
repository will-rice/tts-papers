---
arxiv_id: '2201.07438'
title: 'MHTTS: Fast multi-head text-to-speech for spontaneous speech with imperfect
  transcription'
authors:
- Dabiao Ma
- Yitong Zhang
...
1.  1.
    Text representations $`h`$ contains minimal speaker specific information.
2.  2.
    Text representations $`h`$ should contain as much information about $`A`$ as possible, if the first condition is satisfied.
3.  3.
    Speaker specific blocks $`G`$ has the capacity to reconstruct $`A`$ from $`h`$.
...
2.  2.
    Target corpus, a well-known open-source high-quality mandarin corpus^(††\dagger)^(††\dagger)$`\dagger`$https://www.data-baker.com which contains 10000 male 48 KHz recordings with correct transcription. All recordings are converted to 16 kHz.
